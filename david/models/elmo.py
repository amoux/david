...
def model_file(model: str = None, dirpath="models", override=False):
    """Return a numpy.ndarray if the model name and path exist.

    model: A string from the name of the var object is used for
        saving and loading the npy file.
...
def embed_module(iterable_obj: list):
    """Load the embedded module from Tensorflow HUB."""
...
def elmo_search(query: str, n_results: int, sents: list, embed_sents: object):
    """Semantic search engine from ELmo embeddings."""
    ...
    similar_docs = pd.Series(cosine_similarity(embed_query, embed_sents).flatten())
