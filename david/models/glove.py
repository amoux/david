...
class GloVe:
    ...
    @classmethod
    def load_vocabulary(cls, ndim: str = "100d") -> Dict[str, np.float]:
        """Return the built GloVe embeddings given a available dimension."""
        vocab_file = cls.vocab_files[ndim]
        return cls.build_vocabulary(vocab_file)

    @classmethod
    def fit_embeddings(cls, vocab_index: Dict[str, int], ndim = "100d",
                       vocab_size: int = None):
        ...
        embedding_index = cls.load_vocabulary(ndim)
...
class GloVeTwitter(GloVe):
    """Twitter Vectors for Word Representation.

    Available: ['25d', '100d', '200d', '300d']
    Returns an embedded matrix containing the pretrained word embeddings.
    """

    vocab_files = _load_vocab_files("glove.twitter.27B")
