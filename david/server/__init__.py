...
def check_file_exists(file_name, file_path, file_ext=".db"):
...
def is_file_in_dirpath(file, path):
...
class CommentsSQL(object):
    ...
    def __init__(self, sql_file=None,  sql_path=None, table_name=None):
        self.sql_file = sql_file
        self.sql_path = sql_path
        self.table_name = table_name
        # Downloads db files if they dont exists for the user.
        if self.sql_file and sql_path is None:
            temp_sql_file, david_sqlite_home = (
            ...
            if temp_sql_file in self.AVAILABLE_DATABASES.keys():
                ...
                # Load the database after downloading or if
                # the database was found in the in the directory:
        ...
        if self.sql_path and self.sql_file is None:
            ...
            self.sql_file = os.path.basename(self.sql_path)
