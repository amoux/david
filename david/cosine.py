...
def build_feature_matrix(
    ...
    `ngram` (tuple, int): Choose a ngram for the document frequency.
    ...
    if feature_type == 'binary':
        vectorizer = sklearn.feature_extraction.text.CountVectorizer(
            binary=True, min_df=min_freq,
            max_df=max_freq, ngram_range=ngram)
    if feature_type == 'frequency':
        vectorizer = sklearn.feature_extraction.text.CountVectorizer(
            binary=False, min_df=min_freq,
            max_df=max_freq, ngram_range=ngram)
...
def cosine_similarity(vec, features, num_results=3):
    ...
    similar = numpy.dot(vec, features.T)
    top_similar = similar.argsort()[::-1][:num_results]
    top_scores = [(i, round(similar[i], 3)) for i in top_similar]
    return top_scores


def get_similar_docs(queries, docs, num_results, tfid_vectorizer,
                     tfid_features):
    ...
    tfid_vectorizer : sklearn text TfidfVectorizer
    tfid_features : sklearn csr_matrix from TfidVectorizer
    ...
    for i, query in enumerate(queries):
        ...
        similar_docs = cosine_similarity(
            vec=vec_features, features=tfid_features, num_results=num_results)
