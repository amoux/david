...
def lossFun(inputs: list, targets: list, hprev):
    ...
    `hprev` : (array)
        Is Hx1 array of initial hidden state.
    """
    xs, hs, ys, ps = {}, {}, {}, {}
    hs[-1] = np.copy(hprev)
    loss = 0
...
def sample(h: int, seed_ix: list, n: int):
...
while True:
    # prepare inputs (we're sweeping from left
    # to right in steps seq_length long)
